...
class ArrangeArray:
    ...
    def getFirstEmpty(self):
        if not self._first_empty:
            self.add()
        return self._arrange[self._first_empty]


def arrange_objects_all_build_plates(nodes: List["SceneNode"], min_offset = 8) -> Any:
    ...
    global_container_stack = Application.getInstance().getMachineManager().getActiveMachine().global_stack
